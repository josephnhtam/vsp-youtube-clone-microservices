...
namespace Subscriptions.API.Application.Commands.Handlers {
    public class CreateOrUpdateUserProfileCommandHandler : ICommandHandler<CreateOrUpdateUserProfileCommand> {
        ...
        public async Task<Unit> Handle (CreateOrUpdateUserProfileCommand request, CancellationToken cancellationToken) {
            ...
            if (request.UpdateIfExists) {
                try {
                    await _unitOfWork.ExecuteOptimisticUpdateAsync(async () => {
                        ...
                        if (userProfile != null) {
                            if (userProfile.Update(request.DisplayName, request.Description, request.Handle, request.ThumbnailUrl, request.Version)) {
            ...
            if (create) {
                try {
                    var userProfile = UserProfile.Create(request.Id, request.DisplayName, request.Description, request.Handle, request.ThumbnailUrl, request.Version);
