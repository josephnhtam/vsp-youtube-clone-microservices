...
namespace Library.API.Application.Queries.Handlers {
    public class GetLikedPlaylistQueryHandler : IQueryHandler<GetLikedPlaylistQuery, PlaylistDto> {

        private readonly IUniquePlaylistRepository<LikedPlaylist, PlaylistItem> _repository;
        private readonly IDtoResolver _dtoResolver;

        public GetLikedPlaylistQueryHandler (IUniquePlaylistRepository<LikedPlaylist, PlaylistItem> repository, IDtoResolver dtoResolver) {
            _repository = repository;
            _dtoResolver = dtoResolver;
        }

        public async Task<PlaylistDto> Handle (GetLikedPlaylistQuery request, CancellationToken cancellationToken) {
            LikedPlaylist? playlist = null;
            ...
            return await _dtoResolver.ResolvePlaylistDto<LikedPlaylist, PlaylistItem>(playlist, request.UserId, startPosition, cancellationToken);
        }
